    ...
    , workspace_watcher_socket(new QLocalSocket(this))
{
    ...
    connect(timer, &QTimer::timeout, this, &Logger::requestInputEvents);
    timer->start(1000);

    QTimer* file_save_timer = new QTimer(this);
    connect(file_save_timer, &QTimer::timeout, this, &Logger::save_input_events_to_database);
    file_save_timer->start(5000);

    connect(input_events_socket, &QLocalSocket::readyRead, this, &Logger::readInputEvents);
    connect(input_events_socket, &QLocalSocket::errorOccurred, this, &Logger::displayError);
    ...
    //connect(workspace_watcher_timer, &QTimer::timeout, this, &Logger::requestWorkspaceWatcherEvents);
    connect(workspace_watcher_timer, &QTimer::timeout, this, &Logger::local_socket_state);
...
/*  */
    ...
    connect(workspace_watcher_socket, &QLocalSocket::readyRead, this, &Logger::readWorkspaceWatcher);
    connect(workspace_watcher_socket, &QLocalSocket::disconnected, this, &Logger::requestWorkspaceWatcherEvents);
    connect(workspace_watcher_socket, &QLocalSocket::errorOccurred, this, &Logger::displayError);


}

void Logger::local_socket_state()
{
...
void Logger::requestInputEvents()
{
    blockSize = 0;
...
void Logger::requestWorkspaceWatcherEvents()
{
...
void Logger::save_input_events_to_database()
{
...
void Logger::data_to_json(QString& data)
{
...
void Logger::readWorkspaceWatcher()
{
...
void Logger::displayError(QLocalSocket::LocalSocketError socketError)
{
    switch (socketError) {
